\subsection{Procesos Windows}

Tras comprender el modelo de memoria virtual, el siguiente paso es analizar cómo se organiza la ejecución de las aplicaciones sobre ese espacio de direcciones. En Windows, esta organización se materializa en la abstracción de \textbf{proceso}, que actúa como la unidad básica de ejecución y como contenedor de todos los recursos asociados a un programa en ejecución.

La relación con la memoria virtual es directa: cada proceso dispone de su propio espacio de direcciones aislado, definido por el sistema operativo mediante mecanismos de paginación y control de permisos. Este aislamiento no solo protege la memoria entre procesos, sino que también condiciona cómo se estructuran y gestionan los recursos de ejecución.

Un proceso en Windows no ejecuta instrucciones por sí mismo, sino que proporciona el contexto necesario para que uno o varios hilos puedan hacerlo. Este contexto incluye, entre otros elementos, el espacio de direcciones virtual, los recursos del sistema asignados y la información necesaria para coordinar la ejecución.

En este apartado se describen los componentes principales de un proceso en Windows, con el objetivo de entender cómo se materializa la abstracción introducida por la memoria virtual en la ejecución real de las aplicaciones y cómo esta estructura es fundamental tanto para el funcionamiento del sistema como para el análisis de comportamiento en escenarios de seguridad.

A continuación se describen los principales componentes internos de un proceso en Windows, tanto desde la perspectiva del kernel como del espacio de usuario, con el objetivo de entender cómo se materializa esta abstracción en la arquitectura del sistema operativo.

\begin{itemize}
    \item \textbf{EPROCESS:}  
    Estructura interna del kernel que representa a un proceso. Contiene información crítica como el identificador del proceso (PID), la lista de hilos asociados, permisos, límites de memoria, una referencia al \textit{Process Environment Block} (\acrshort{peb}), así como metadatos utilizados por el planificador y el gestor de seguridad. Constituye la representación fundamental del proceso en modo kernel.
    ...
    \item \textbf{Módulos (\acrshort{dll}s):}  
    Lista de módulos y bibliotecas cargadas en el proceso, mantenida en el \textit{\acrshort{peb} Ldr}.
\end{itemize}


\subsection{Creación de un proceso (Kernel)}

La creación de un proceso en Windows es un procedimiento complejo que se realiza íntegramente a nivel del kernel y sigue una secuencia bien definida de inicialización y configuración de estructuras internas. En este proceso se establecen todos los elementos necesarios para que el sistema operativo pueda gestionar su ejecución, incluyendo su espacio de direcciones virtual, sus estructuras de control y los recursos asociados.

Para una explicación detallada y rigurosa de este mecanismo, se recomienda la exposición de Duncan Ogilvie, donde se describe de forma clara y precisa el flujo interno mediante el cual Windows inicializa y pone en ejecución un nuevo proceso \cite{ogilvie_windows_internals_crash_course}.
...
\begin{itemize}[leftmargin=1.5cm]
    ...
    \item \textbf{Asignación del \acrshort{peb} (Process Environment Block):}  
...
El \textbf{Process Environment Block (\acrshort{peb})} es una estructura interna de Windows que almacena información esencial sobre un proceso en ejecución. Aunque Microsoft no publica una definición oficial completa de esta estructura, su diseño es conocido gracias al análisis inverso y puede variar ligeramente entre versiones del sistema operativo. El \acrshort{peb} reside en \textit{user mode} y es único para cada proceso.

Entre los elementos más relevantes del \acrshort{peb} se encuentran:
...
\begin{itemize}
    ...
    \item \textbf{Ldr (PEB\_LDR\_DATA):} Estructura que mantiene la lista de módulos cargados (\acrshort{dll}s).  
...
Una explicación detallada y ejemplos de cómo iterar el \acrshort{peb}, tanto desde C como desde WinDbg, pueden encontrarse en \cite{nyascla_walking_peb}.

\subsubsection*{Iterar el PEB para recuperar la base de un módulo}

El procedimiento estándar para recorrer el \acrshort{peb} y obtener la base de una \acrshort{dll} concreta es el siguiente:
...
\subsection{Genealogía de Procesos en Windows}
